...
Le mer. 23 sept. 2026, 20:23, Idriss KISITA <mrkisita.idriss@outlook.fr> a écrit :
const { createClient } = require('@supabase/supabase-js');
const crypto = require('crypto');
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (secret) {
      ...
      if (signature !== digest) {
        console.error('Invalid signature:', signature, 'expected:', digest);
    ...
    console.log('Order status:', status, 'Email:', customerEmail);

    // Accept paid or pending (test mode uses 'paid')
    if (status !== 'paid' && status !== 'pending') {
      return { statusCode: 200, body: `Order status ${status} ignored` };
    }

    // Detect pack from order total (in cents)
    let pack = null;
    const items = order?.order_items || [];
    for (const item of items) {
      const vid = String(item.variant_id);
      if (PACKS[vid]) { pack = PACKS[vid]; break; }
    }
    ...
    const code = generateCode(pack.name);
    console.log('Generated code:', code, 'Pack:', pack.name);
    ...
    console.log('Code saved to Supabase successfully');

    // Send email via Brevo
    const brevoKey = process.env.BREVO_API_KEY;
    console.log('Brevo key present:', !!brevoKey, 'Customer email:', customerEmail);
    ...
    if (brevoKey && customerEmail) {
      ...
      const brevoBody = await brevoRes.text();
      console.log('Brevo response status:', brevoRes.status, 'body:', brevoBody);

      if (!brevoRes.ok) {
        console.error('Brevo email failed:', brevoRes.status, brevoBody);
        // Still return 200 — code was saved, email failed
        return { statusCode: 200, body: JSON.stringify({ success: true, code, emailError: brevoBody }) };
      }
    } else {
      console.warn('Brevo not configured or no customer email');
    }
